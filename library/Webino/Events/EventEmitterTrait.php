...
trait EventEmitterTrait
{
     ...
     * @throws Exceptions\InvalidArgumentException Invalid event
     * @throws Exceptions\RuntimeException Missing event name
     */
    public function emit($event, array $params = [], callable $until = null) : EventInterface
    {
        ...
        if (empty($name)) {
            throw (new Exceptions\RuntimeException('Cannot emit event %s missing a name'))
                ->format($event);
        }
    ...
    protected function normalizeEvent($event) : ?EventInterface
    {
        ...
        throw (new Exceptions\InvalidArgumentException('Expected event as %s but got %s'))
